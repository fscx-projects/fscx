...
type private FscxInjectAspectVisitorImpl<'TAspect> private () = 
    ...
    static let assertLookup f name = 
        let mi = f name
        if mi = null then raise (InvalidOperationException name)
        else mi
    
    static let enterMethod = 
        assertLookup 
            (fun name -> 
            typeof<'TAspect>.GetMethod(name, BindingFlags.Public ||| BindingFlags.Static ||| BindingFlags.DeclaredOnly)) 
            "Enter"
    static let leaveMethod = 
        assertLookup 
            (fun name -> 
            enterMethod.ReturnType.GetMethod
                (name, BindingFlags.Public ||| BindingFlags.Instance ||| BindingFlags.DeclaredOnly)) "Leave"
    static let caughtMethod = 
        assertLookup 
            (fun name -> 
            enterMethod.ReturnType.GetMethod
                (name, BindingFlags.Public ||| BindingFlags.Instance ||| BindingFlags.DeclaredOnly)) "Caught"
...
type FscxInjectAspectVisitor<'TContext when 'TContext : (new : unit -> 'TContext)> private (aspectEnter : string list) = 
    ...
    static let constUnit = SynExpr.Const(SynConst.Unit, zeroRange)
    static let constString value = SynExpr.Const(SynConst.String(value, zeroRange), zeroRange)
    static let constInt32 value = SynExpr.Const(SynConst.Int32(value), zeroRange)
    
    static let createIdent (idents : string list) range = 
        match idents with
        | [ ident ] -> SynExpr.Ident(Ident(ident, range))
        | _ -> 
            SynExpr.LongIdent
                (false, 
                 LongIdentWithDots
                     (idents |> List.map (fun ident -> Ident(ident, zeroRange)), 
                      List.init (idents |> List.length) (fun _ -> zeroRange)), None, range)
    
    static let createTuple exprs = 
        SynExpr.Tuple(exprs, 
                      exprs
                      |> List.skip 1
                      |> List.map (fun _ -> zeroRange), zeroRange)
    
    static let createApp idents appExpr = 
        SynExpr.App
            (ExprAtomicFlag.NonAtomic, false, createIdent idents zeroRange, 
             SynExpr.Paren(appExpr, zeroRange, None, zeroRange), zeroRange)
    static let createSequence exprs = 
        exprs 
        |> Seq.reduceBack 
            (fun expr0 expr1 -> 
                SynExpr.Sequential
                    (SequencePointInfoForSeq.SuppressSequencePointOnStmtOfSequential, true, expr0, expr1, zeroRange))
    ...
    static let createMatchClause pattern expr = 
        SynMatchClause.Clause(pattern, None, expr, zeroRange, SequencePointInfoForTarget.SuppressSequencePointAtTarget)
            ...
            (false, false, 
                [ SynBinding.Binding
                    (None, SynBindingKind.NormalBinding, false, false, [], PreXmlDoc.Empty, SynValData.Empty, 
                        createNamedPattern name, None, bindExpr, zeroRange, spBind) ], bodyExpr, zeroRange)
    ...
    static let rec identForSymbol expr = 
        ...
        | SynExpr.App(_, _, funcExpr, _, _) -> identForSymbol funcExpr
        // Single symbol
        | SynExpr.Ident ident -> Some(expr, [ ident ], ident.idText)
    ...
    static let (|IdentForSymbol|_|) expr = identForSymbol expr
    ...
    static let rec argumentsByCurrying funcExpr argExpr argExprs = 
        ...
        // Curryable arguments
        | SynExpr.App(_, _, funcExpr, argExpr, _), _ -> argumentsByCurrying funcExpr argExpr argExprs
        // Other
        | _ -> argExprs
    ...
    static let (|Arguments|) (funcExpr, argExpr) = 
        match funcExpr, argExpr with
        // Curryable arguments
        | SynExpr.App _, _ -> (argumentsByCurrying funcExpr argExpr []), true // currying
        // Tuple arguments
        | _, SynExpr.Paren(SynExpr.Tuple(exprs, _, _), _, _, _) -> exprs, false
        // Argument nothing (Paren only)
        | _, SynExpr.Const(SynConst.Unit, _) -> [], false
        // Other
        | _ -> [ argExpr ], false
    ...
    static let createArgumentExpr origExpr exprs = 
        ...
        | SynExpr.Paren(_, _, _, _), [ expr ] -> SynExpr.Paren(expr, zeroRange, None, zeroRange)
        | SynExpr.Const(SynConst.Unit, _), _ -> SynExpr.Const(SynConst.Unit, zeroRange)
        | _, [ expr ] -> expr
        | _ -> failwith ""
    
    static let createArrayWithArgs exprs = 
        SynExpr.ArrayOrList
            (true, exprs |> List.mapi (fun index _ -> createIdent [ getArgName index ] zeroRange), zeroRange)
    ...
    static let getSymbolUseAtIdent (symbolInformation : FSharpCheckFileResults) (ids : Ident list) = 
        ...
        symbolInformation.GetSymbolUseAtLocation(head.StartLine, last.EndColumn, text, names)
    
    //////////////////////

    // Public constructor
    new(aspectTypeName : string) = 
        FscxInjectAspectVisitor([ yield! aspectTypeName.Split('.')
                                  yield "Enter" ])
    ...
    member private __.InsertAspectToAppExpr(identExpr, symbolName, argExpr, deconstructedExprs, currying, 
        ...
        let innerBodyApp = 
            ...
            let argExprs = deconstructedExprs |> List.mapi (fun index _ -> createIdent [ getArgName index ] zeroRange)
            ...
            if currying then 
                ...
                //   Bound arguments named with "__arg_n".
                List.foldBack 
                    (fun expr lastExpr -> SynExpr.App(ExprAtomicFlag.NonAtomic, false, lastExpr, expr, zeroRange)) 
                    (argExprs |> List.rev) identExpr
            else 
                // Step1-2:
                //   Construct bound arguments.
                //   Bound arguments named with "__arg_n".
                SynExpr.App
                    (ExprAtomicFlag.NonAtomic, false, identExpr, argExprs |> createArgumentExpr argExpr, zeroRange)
        
        // Step3:
        //  Invoke __context.Leave.
        let leaveApp = createApp [ "__context"; "Leave" ] innerBodyApp

        // Step4:
        //   Invoke __context.Caught.
        let caughtApp = createApp [ "__context"; "Caught" ] (createIdent [ "ex" ] zeroRange)

        // Step5:
        //   "reraise()"
        let reraiseApp = createApp [ "reraise" ] constUnit

        // Step6:
        //   Combined with-block exprs
        let withSequence = createSequence [ caughtApp; reraiseApp ]

        // Step7:
        //   Construct with clause
        let withClause = createMatchClause (createNamedPattern "ex") withSequence

        // Step8:
        //   Construct try-with
        let tryWith = 
            SynExpr.TryWith
                (leaveApp, zeroRange, [ withClause ], zeroRange, zeroRange, SequencePointInfoForTry.NoSequencePointAtTry, 
                 SequencePointInfoForWith.NoSequencePointAtWith)

        // Step9:
        //   [|__arg_0;__arg_1;__arg_2;__arg_3|]
        let argsExpr = createArrayWithArgs deconstructedExprs
        
        // Step10:
        //   Aspect.Enter("System.String.Format", "Sample.fs", 12, 34, [|__arg_0;__arg_1;__arg_2;__arg_3|])
        let enterApp = 
            createApp aspectEnter (createTuple [ constString symbolName
                                                 constString appRange.FileName
                                                 constInt32 appRange.StartLine
                                                 constInt32 appRange.StartColumn
                                                 argsExpr ])
        
        // Step11:
        //   let __context = Aspect.Enter(...) in try...
        let contextBound = 
            createLetBinding "__context" enterApp tryWith SequencePointInfoForBinding.NoSequencePointAtLetBinding
        ...
        let generatedExpr = 
            List.foldBack 
                ...
                createLetBinding (getArgName index) expr lastExpr 
                    (if index = 0 then SequencePointInfoForBinding.SequencePointAtBinding appRange
                     else SequencePointInfoForBinding.NoSequencePointAtLetBinding)) 
                (deconstructedExprs |> List.mapi (fun index expr -> index, expr)) contextBound
    ...
    /// </summary>
    override __.BeforeVisitBinding_Binding(context, access, bindingKind, mustInline, isMutable, attributes, xmlDoc, 
                                           item7, headPat, item9, expr, lhsRange, spBind) = 
        let newContext = 
            // Check if applied attributes:
            let found = 
                attributes.AsParallel().Any(new Func<SynAttribute, bool>(fun attribute -> 
                          // Binding expression has one or more attributes
                          let ids = attribute.TypeName.Lid
                          let result = getSymbolUseAtIdent context.SymbolInformation ids |> Async.RunSynchronously // Cannot async wait
                          match result with
                          | Some symbolUse -> 
        ...
        base.BeforeVisitBinding_Binding
            (newContext, access, bindingKind, mustInline, isMutable, attributes, xmlDoc, item7, headPat, item9, expr, 
             lhsRange, spBind)
    
    /// <summary>
    /// Hook "SynExpr.App" (Before visit)
    /// </summary>
    override this.BeforeVisitExpr_App(context, exprAtomicFlag, isInfix, funcExpr, argExpr, appRange) = 
            ...
            | IdentForSymbol(identExpr, ids, symbolName), Arguments(deconstructedExprs, currying) -> 
                ...
                | Some symbolUse -> 
                    ...
                    let foundForQualified = 
                        context.TargetAssemblies |> Seq.exists (fun regex -> regex.IsMatch qualifiedName)
                    let found = 
                        foundForQualified 
                    ...
                    if found then 
                        ...
                        // Insert aspect
                        this.InsertAspectToAppExpr
                            (identExpr, symbolName, argExpr, visitedDeconstractedExprs, currying, appRange)
                    else 
                        // Continue visit
